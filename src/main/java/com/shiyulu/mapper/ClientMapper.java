...
public interface ClientMapper {
    //查询全部的客户信息，主要用于后台展示
    @Select("select * from repaircar.client")
    List<Client> selectAllClientInfo();

    //根据客户ID查询某个客户的信息
    @Select("select * from repaircar.client where clientId = #{clientId}")
    Client selectClientInfoById(Integer clientId);

    //查询全部客户的车辆信息，主要用于后台展示
    @Select("select * from repaircar.vehicle")
    List<Vehicle> selectAllVehicleInfo();

    //根据客户ID查询某个客户的车辆信息
    @Select("select * from repaircar.vehicle where clientId = #{clientId}")
    List<Vehicle> selectVehicleInfoByClientId(Integer clientId);

    //查询全部客户的车辆维修信息，主要用于后台展示
    @Select("select * from repaircar.vehiclefault")
    List<VehicleFault> selectAllVehicleFaultInfo();

    //根据车架号查询维修车辆信息
    @Select("select * from repaircar.vehiclefault where vin = #{vehicleIdentNum}")
    ...
    //根据维修订单号查询维修车辆信息
    @Select("select * from repaircar.vehiclefault where vfi = #{vehicleFaultId}")
